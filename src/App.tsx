...
const CASH_PRIZE_IMAGE = IMG_cashPrize || "/images/posters/cash-price.png";
...
export default function App() {
  ...
  const fetchRegistrations = async () => {
    try {
      ...
      try {
        ...
        // server API may not exist in pure client build
      }

      // 2. Fetch directly from Firebase Firestore
      const firestoreList = await fetchRegistrationsFromFirestore();
      if (firestoreList.length > 0) {
        const map = new Map();
        [...firestoreList, ...list].forEach(item => {
          const key = item.whatsappNumber || item.whatsAppNumber || item.id;
          if (key && !map.has(key)) {
            map.set(key, {
              ...item,
              ...
              previousCoaching: item.attendedCoachingBefore || item.previousCoaching
            });
          }
        });
  ...
  useEffect(() => {
    ...
    const fetchSheetsConfig = async () => {
      try {
        // Try Firestore first
        const firestoreConfig = await fetchSheetsConfigFromFirestore();
        if (firestoreConfig) {
          setSheetsConfig({
            isConnected: true,
            spreadsheetId: firestoreConfig.spreadsheetId,
            spreadsheetUrl: firestoreConfig.spreadsheetUrl,
            sheetTitle: firestoreConfig.sheetTitle,
            updatedAt: firestoreConfig.updatedAt
          });
          return;
        }
